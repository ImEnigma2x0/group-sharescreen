...
function describe(rule: AutoFlagRule, plans: AdminPlanOption[]): string {
  ...
  if (rule.from !== null && rule.to !== null) {
    return `Quem ${plan} entre ${when(rule.from)} e ${when(rule.to)}`;
  }
  if (rule.to !== null) return `Quem ${plan} antes de ${when(rule.to)}`;
  return `Quem ${plan} a partir de ${when(rule.from)}`;
}
...
export function AutoFlagsPanel() {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setFlag("");
      setLabel("");
      setDone("Regra criada. Use “Aplicar agora” para alcançar quem já existe.");
    } catch (err) {
      setError(err instanceof Error ? err.message : "Falha ao criar a regra.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function handleRun() {
    ...
    try {
      ...
      setDone(
        result.grants === 0
          ? "Nenhuma conta nova se encaixou nas regras."
    ...
    <div className="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-950">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
            Dão uma flag sozinhas para quem se encaixa. Uma regra só{" "}
            <span className="font-medium">concede</span>: desligar ou apagar para de dar, e não
            tira de quem já recebeu.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="mt-4 flex flex-col gap-3 rounded-lg border border-zinc-200 p-3 dark:border-zinc-800">
        <div className="flex flex-wrap gap-2">
          {(
            [
              { id: "signup", label: "Criou a conta" },
              { id: "subscription", label: "Assinou um plano" },
            ] as const
          ).map((option) => (
            <button
              key={option.id}
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <label className="flex flex-col gap-1 text-xs font-medium text-zinc-600 dark:text-zinc-400">
            ...
            <input
              ...
              placeholder="Apoiadores iniciais"
              className={inputClass}
            />
          </label>
        </div>
      ...
      {/* The rules there are */}
      {rules.length === 0 ? (
        <p className="mt-4 text-sm text-zinc-500 dark:text-zinc-400">Nenhuma regra ainda.</p>
      ) : (
          ...
          {rules.map((rule) => (
            ...
            >
              ...
              <button
                type="button"
                onClick={() => {
                  if (!window.confirm("Apagar a regra? Quem já recebeu a flag continua com ela."))
                    return;
